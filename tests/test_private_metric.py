...
class PrivateMetricTests(unittest.TestCase):
    ...
    def test_fork_sets_parent_to_best(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("MODE=mechanical-no-headless", fork.stdout)
            self.assertIn("SCHEMA_VERSION=2", fork.stdout)
            self.assertIn("OUTPUT_DIR=", fork.stdout)
            self.assertIn("PARENT=002", fork.stdout)
            self.assertIn("BEST_EXPERIMENT=002", fork.stdout)
            self.assertIn("LINEAGE=explore-alt", fork.stdout)
            self.assertIn("REVERTED=false", fork.stdout)
            state = json.loads((work / "autoresearch-results" / "state.json").read_text(encoding="utf-8"))
